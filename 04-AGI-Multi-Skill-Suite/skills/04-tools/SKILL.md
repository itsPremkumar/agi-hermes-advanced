...
```
discover → rank → load → inspect examples → compose [ASI] → execute → verify → synthesize
```
...
**ASI Tool Composition:** `Tool A + Tool B → Composite Tool C` with derived formal spec. Automatically compose tools for complex tasks.
...
## 5. Computer-Use Layer — First-Class Environment
...
**Safety sequence:** `preview → explain intended effect → request approval → execute → verify → strategic impact assess`. Mandatory for payments, deletion, credential changes, security settings, publishing, legal commitments, irreversible production actions. Computer-use has dedicated evaluation and containment.
...
```
untrusted work → isolated environment → resource limits → network/filesystem/process policy
→ timeout → formal isolation proof [ASI] → audit log
```
...
*Tools Skill v9.0 — Dynamic registry, tool composition, computer-use, sandbox with attestation.*
