...
description: >
  Hermes AI Agent — Internet Search Super-Optimized Execution Protocol.
...
# HERMES — Internet Search Optimized Protocol v1.0
...
## 0. PURPOSE — SEARCH AS SUPERPOWER
...
## 1. HERMES SEARCH TOOLSET — OPTIMIZED CONFIG
...
```yaml
# Model — needs >=64K context for deep research
...
memory:
  ...
  memory_char_limit: 4000       # INCREASED for search (was 2200) — stores evidence graphs
...
toolsets:
  enabled:
    - web_search               # REQUIRED — primary search
    - browser                  # REQUIRED for search-heavy — renders JS, bypasses snippet limits
    - file_read
...
2. Confirm web_search is available — if not, STOP and report CAPABILITY_UNAVAILABLE
...
Standard loop is `RECEIVE → RESEARCH → PLAN → EXECUTE → VERIFY`. For search-heavy Hermes, it becomes:
...
DEEP EXTRACTION (browser load full pages, not snippets)
  ↓
EVIDENCE GRAPH (Claim → Source → Evidence → Counter-evidence → Confidence)
  ↓
CONTRADICTION SEARCH (actively seek disconfirming evidence)
  ↓
SYNTHESIS + GAP DETECTION (what is still unknown?)
  ↓
SECOND SEARCH WAVE (if gaps remain — new queries from synthesis)
  ↓
...
## 3. QUERY COMPILATION — HOW HERMES TURNS ONE QUESTION INTO 5 SEARCHES
...
1.  **One search per sub-question** — Decompose complex question into atomic searchable facts
2.  **Add site: and date filters** — `site:official-docs`, `after:2025-01-01`, `2026`
3.  **Vary query phrasing** — Same fact with 2 different phrasings catches different sources
4.  **Include counter-query** — One search should seek disconfirming evidence: `"hermes agent limitations"`, `"hermes agent security issues"`
5.  **Parallelize** — Fire 3-5 searches simultaneously via Hermes parallel tool calling (not sequentially)
...
## 4. SOURCE TRIAGE — RANKING WHAT HERMES FOUND
...
## 5. DEEP EXTRACTION — BEYOND SNIPPETS
...
```
web_search → get top 5 URLs → browser load each full page in parallel
  → extract: {title, publish_date, author, main_content, code_blocks, tables}
  → check: is this the primary source or a summary of another source?
  → if summary: follow link to primary and load that too
  → save evidence to file: ./evidence/{claim_id}.md with full provenance
```
...
## 6. EVIDENCE GRAPH — HERMES FILE-BASED
...
```
./evidence/
├── evidence-graph.md          # Master: Claim → Source → Confidence table
├── sources.md                 # All sources with reliability scores
├── contradictions.md          # Conflicting claims preserved
└── raw/
    ├── source-01-hermes-docs.md
    ├── source-02-github.md
    └── source-03-paper.md
```
...
| 2 | Hermes default write_approval is false | config.yaml docs | Primary | 2026 | 0.90 | strongly_supported | — |
```

---

## 7. CONTRADICTION SEARCH — HERMES RED-TEAM
...
Action: Browser load contradictory source → compare claims side-by-side
...
## 8. SECOND SEARCH WAVE — GAP CLOSURE
...
→ Generate NEW targeted searches for those gaps
→ Fire second parallel wave (2-3 searches)
→ Merge results into evidence graph
```
...
## 10. FINAL REPORT — HERMES SEARCH DELIVERABLE
...
## 11. HERMES SEARCH TUNING — PERFORMANCE TIPS
...
*   This skill optimizes **SEARCH**. It does NOT replace `SOUL.md` (values, safety) — keep SOUL.md loaded.
*   This skill does NOT give Hermes new capabilities — it organizes existing `web_search` + `browser` + `file_*` tools into a superior workflow.
...
*Hermes Search-Optimized v1.0 — Pairs with SOUL.md v4.0 ASI. All search concepts from AGX, Hermes, and Deep Harness consolidated and super-optimized for internet search on Hermes.*
